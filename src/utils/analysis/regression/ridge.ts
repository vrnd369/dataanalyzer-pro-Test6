...
export function calculateRidgeRegression(
  ...
  // Add L2 regularization term (ridge penalty)
  const XtX = multiplyMatrix(transposeMatrix(X), X);
  const identityMatrix = Array(XtX.length).fill(0).map((_, i) => 
    Array(XtX.length).fill(0).map((_, j) => i === j ? alpha : 0)
  );
  
  // Add regularization term to XtX
  const regularizedXtX = XtX.map((row, i) => 
    row.map((val, j) => val + identityMatrix[i][j])
  );
  
  const Xty = multiplyMatrix(transposeMatrix(X), y.map(yi => [yi]));
  const coefficients = solveLinearSystem(regularizedXtX, Xty).map(row => row[0]);
